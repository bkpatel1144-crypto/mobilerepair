...
/**
 * This app's nav (section key + leaf slug) against the reference's feature slug.
 *
 * Where the two disagree it is only ever spelling — `options` here is `service-options` there.
 * The two report entries are worth pausing on: the export labels `REPORTS_PROFIT_LOSS_VIEW` as
 * "View Profit & Loss" and `REPORTS_PNL_REPORT_VIEW` as "View Job-wise Profit", which is the
 * opposite of what the key names suggest. The labels win — they are what a person reads.
 */
const FEATURE_MAP = {
