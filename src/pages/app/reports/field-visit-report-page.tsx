...
export function FieldVisitReportPage() {
  ...
  const filtered = visits
    .filter(
      (v) =>
        !bounds ||
        ((v.createdAt?.toDate?.() ?? new Date()) >= bounds.from &&
          (v.createdAt?.toDate?.() ?? new Date()) <= bounds.to)
    )
  ...
  const allDeviceTypes = Array.from(
    new Set(visits.map((v) => v.deviceTypeName).filter((v): v is string => !!v))
  ).sort()
  ...
  function buildTechnicianGroups(): TechnicianVisitGroup[] {
    ...
    for (const group of map.values())
      group.jobsVisited = new Set(group.visits.map((v) => v.jobCardId)).size
  ...
  function buildJobGroups(): JobVisitGroup[] {
    ...
    return Array.from(map.values())
      .filter((g) =>
        search.trim()
          ? `${g.jobNumber} ${g.customerName}`.toLowerCase().includes(search.toLowerCase())
          : true
      )
      .sort((a, b) => b.visits.length - a.visits.length)
  }
  ...
  const jobColumns: DataTableColumn<JobVisitGroup>[] = [
    {
      ...
      render: (g) => (
        <Link
          to={`/app/service/job-cards/${g.jobCardId}`}
        ...
        >
          <div className="flex gap-1 rounded-lg border p-0.5">
            ...
            <Button
              type="button"
              size="sm"
              variant={view === 'jobCard' ? 'default' : 'ghost'}
